...
export default function Candidatures({ session, isDark, applications, setApplications }) {
  ...
  const handleAdd = async () => {
    ...
    try {
      const response = await fetch('/api/add', {
        ...
        body: JSON.stringify({ ...formData, user: session._id }),
      });
      if (response.ok) {
  ...
  const handleDownload = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.error("Erreur lors du téléchargement des candidatures");
      }
    } catch (error) {
      console.error("Erreur réseau :", error);
    }
  ...
  return (
    <div className={`min-h-screen transition-all duration-700 ${isDark
      ...
      <div className="relative z-10 mx-auto px-4 py-12">
        ...
        <div className="max-w-[1400px] mx-auto mb-6 flex justify-between">
          <button
            onClick={() => setIsAdding(true)}
            className={`px-6 py-3 rounded-xl font-bold flex items-center gap-2 transition-all duration-300 ${isDark
          ...
          >
            ...
            <span className='hidden md:block'>Ajouter une candidature</span>
          </button>
          ...
          >
            ...
            <span className='hidden md:block'>Télécharger CSV</span>
          </button>
        </div>
            ...
            <div className={`rounded-3xl overflow-hidden backdrop-blur-xl ${isDark
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <input
                    type="date"
                    value={formData.dateEnvoi.includes("T") ? new Date(formData.dateEnvoi).toISOString().split("T")[0] : formData.dateEnvoi}
              ...
              <div className="flex gap-3 mt-6">
                ...
                >
                  ...
                  <span className='hidden md:block'>{isAdding ? 'Ajouter' : 'Mettre à jour'}</span>
                </button>
                <button
                  onClick={() => {
                    setIsAdding(false);
